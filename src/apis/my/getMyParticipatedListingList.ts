...
interface IMyParticipatedListingListItem {
  listing_id: number;
...
interface GetMyParticipatedListingListResponse {
  count: number;
  list: IMyParticipatedListingListItem[];
}

function getKey(filter: number) {
  return (size: number, previousPageData: GetMyParticipatedListingListResponse) => {
...
export default function useAPI_GetMyParticipatedListingList(filter: number) {
  const { user } = useAuth();
  const {
    data: dataList,
    size,
    ...
    isLoading,
    mutate,
  } = useSWRInfinite<GetMyParticipatedListingListResponse>(user ? getKey(filter) : () => null);
