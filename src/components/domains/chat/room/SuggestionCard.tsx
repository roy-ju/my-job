...
import { IChatRoomDetailSuggestItem } from '@/services/chat/type';
...
export function SuggestionItemCard({ item }: Omit<SuggestionItemCardProps, 'count'>) {
  const { handleClickNavigateToSuggestDetail } = useClientAccordionHandler();
  ...
  return (
    <button
      type="button"
      tw="w-full [text-align: left] h-[72px] flex flex-row bg-white px-4 py-2 [border-radius: 8px] items-center hover:bg-gray-300 cursor-pointer"
      onClick={() => handleClickNavigateToSuggestDetail?.(item.suggest_id)}
    >
      <div tw="min-w-[60px]">
