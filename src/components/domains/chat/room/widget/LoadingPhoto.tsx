...
const variants = {
  gray: tw`bg-white text-gray-1000`,
  nego: tw`bg-white text-gray-1000`,
...
export function LoadingPhoto(props: Omit<HTMLProps<HTMLDivElement>, 'theme' | 'as'>) {
  const { variant } = useContext(ChatMessageContext);

  return <Container variant={variant} {...props} />;
}

export const LoadingPhotoType = (<LoadingPhoto />).type;
