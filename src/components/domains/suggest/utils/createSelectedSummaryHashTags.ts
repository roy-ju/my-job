...
} from '../form/constants/hashtags';
import isEqualValue from './isEqualValue';

export default function createSelectedSummaryHashTags({
  danjiOrRegion,
  buyOrRent,
  realestateType,
  selectedConditions,
  ...
  if (isEqualValue(danjiOrRegion, DanjiOrRegionalType.Danji)) {
    if (isEqualValue(buyOrRent, BuyOrRent.Buy)) {
      return [{ title: '', list: selectedConditions.filter((ele) => DanjiBuyOptions.includes(ele)).join(' / ') }];
    }

    if (isEqualValue(buyOrRent, BuyOrRent.Jeonsae)) {
      return [{ title: '', list: selectedConditions.filter((ele) => DanjiJeonsaeOptions.includes(ele)).join(' / ') }];
    }
