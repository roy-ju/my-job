...
import SigunguIcon from '@/assets/icons/sigungu_line_green.svg';
import { GetDanjiDetailResponse } from '@/apis/danji/danjiDetail';
import useXAxisDate from '@/components/pages/pc/DanjiDetail/useXAxisDate';
import moment from 'moment';
import { ParentSize } from '@visx/responsive';
import DanjiIcon from '@/assets/icons/danji_line_blue.svg';
import SidoIcon from '@/assets/icons/sido_line_purple.svg';
import { useAPI_DanjiSidoValues, useAPI_DanjiValues } from '@/apis/danji/danjiValues';
...
export const RealPriceValuesChartWrraper = React.memo(
  ({
    danji,
    buyOrRent,
    selectedYear,
    ...
    const { listDanji, listSigungu } = useAPI_DanjiValues({
    ...
    const { listSido } = useAPI_DanjiSidoValues({
      danjiId: danji?.danji_id,
      realestateType: Number(danji?.type),
      buyOrRent,
      year: selectedYear,
      ...
      <div>
        <div tw="flex flex-col [gap: 14px]">
          ...
          <span tw="text-info [line-height: 1] text-gray-700">
            *개별 실거래가격 및 거래되지 않은 평형의 직전 가격을 반영한 평균값 (면적당)
          </span>
        </div>
